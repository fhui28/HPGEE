#' ---
#' title: Template code for running simulations involving semi-continuous (Tweedie) response data; see Section 4 of the associated manuscript "Simultaneous homogeneity pursuit and variable selection in regression models for multivariate abundance data" for more details 
...
library(cluster)  
library(ROCR)
here::i_am("simulations/setting1/setting1_tweedien125.R")
library(here)
...
dosims <- function(NAI) {
  ...
  for(k in 1:num_resp) {
   find_cv <- cv.glmnet(y = simy[,k], x = as.matrix(X[,-1,drop=FALSE]), type.measure = "default", family = statmod::tweedie(var.power = 1.2, link.power = 0))
  ...
  save(all_metrics, all_comptimes, file = paste0("setting1_tweedien",num_units,"_dataset",NAI,".RData"))
  }
