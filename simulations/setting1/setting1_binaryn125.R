#' ---
#' title: Template code for running simulations involving binary response data; see Section 4 of the associated manuscript "Simultaneous homogeneity pursuit and variable selection in regression models for multivariate abundance data" for more details 
...
library(cluster)  
library(ROCR)
here::i_am("simulations/setting1/setting1_binaryn125.R")
library(here)
...
P <- rho^P 
myfam <- binomial(link = "probit")

set.seed(102022)
true_lvcoefs <- matrix(runif(num_resp*num_lv, -1.5, 1.5), nrow = num_resp, ncol = num_lv)
true_disp_param <- runif(num_resp, 0, 4)
marginalization_adjustment <- matrix(sqrt(rowSums(true_lvcoefs^2) + 1), nrow = num_resp, ncol = num_X+1, byrow = FALSE)
...
X_coefs <- X_coefs*marginalization_adjustment # Scaled up due to the connection between marginal and conditional regression coefficients in binary probit GEEa
rm(P, rho, num_lv)
...
dosims <- function(NAI) {
  ...
  true_marginal_coefs <- X_coefs/marginalization_adjustment
  X <- as.data.frame(X)
  ...
  for(k in 1:num_resp) {
    find_cv <- cv.glmnet(y = simy[,k], x = as.matrix(X[,-1,drop=FALSE]), type.measure = "default", family = binomial(link = "probit"))
  ...
  save(all_metrics, all_comptimes, file = paste0("setting1_binaryn",num_units,"_dataset",NAI,".RData"))
  }
