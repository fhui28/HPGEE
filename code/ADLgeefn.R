#' @title Adaptive lasso in GEEs
#' 
#' @description Fits a regularization path for GEEs to achieve sparsity using an adaptive lasso penalty, assuming either an independence, an unstructured, or a reduced-rank working correlation matrix. 
#' 
...
#' 
#' @return A object of class "adlgee_regpath" with the following values, as appropriate:
...
#' \item{singlefit_quantities:}{A list containing quantities that can be passed into \code{adlgee_fit} to construct a single penalized GEE e.g., at a given lambda.}
...
function() {
   ...
   control = list(maxit = 100, tol = 1e-6, step_halving = TRUE, max_halving_steps = 30, trace = TRUE)   
   }

adlgee_regpath <- function(y, X, family = gaussian(), nlambda = 100, lambda_min_ratio = 1e-4, lambda = NULL, min_df = NULL, 
                  kappa = rep(c(0,1), c(1,ncol(X)-1)),
   ...
   penweights_ADL <- 1/abs(fit_ungee$coefficients_matrix)*matrix(kappa, nrow = num_resp, ncol = num_X, byrow = TRUE)
   penweights_ADL <- as.vector(t(penweights_ADL))
   
   ##-----------------------
   ## Attempt to determine a minimum and maximum lambda, if required
   ## .adlgee_fit_manual has a built-in option to get the glmnet regularization path from the final step of the fitting algorithm. Use this as the basis to then determine a maximum lambda, and increase this until you hit an intercept-only model.
   ##-----------------------
   if(!is.null(lambda))
      lambdaseq <- lambda
   if(is.null(lambda)) {
      message("Finding an appropriate sequence of lambda values...")
      fit_togetmaxlambda <- .adlgee_fit_manual(yvec = yvec, bigX = bigX, lambda = 0, penalty_weights = penweights_ADL, 
                                          num_resp = num_resp, num_units = num_units, getalambdaseq = TRUE,
      ...
      trynullfit <- .adlgee_fit_manual(yvec = yvec, bigX = bigX, lambda = lambda_max, penalty_weights = penweights_ADL, 
                           num_resp = num_resp, num_units = num_units, 
                           family = family, corstr = corstr, rank = rank, response_names = colnames(y), control = control)
   ...
   if(!multicore_path) {
      ...
      for(l in 1:length(lambdaseq)) {
         cwfit <- .adlgee_fit_manual(yvec = yvec, bigX = bigX, lambda = lambdaseq[l], penalty_weights = penweights_ADL, 
                                 num_resp = num_resp, num_units = num_units, 
                                 family = family, corstr = corstr, rank = rank, response_names = colnames(y), control = control)
   ...
   if(multicore_path) {
      ...
      fit_multicore_path <- foreach(l = 1:length(lambdaseq)) %dopar% adlgee_outerfn(l = l)
   ...
   out$singlefit_quantities <- list(yvec = yvec, bigX = bigX, penalty_weights = penweights_ADL, 
                                    control = control, response_names = colnames(y), covariate_names = colnames(X))
   
   class(out) <- "adlgee_regpath"
   return(out)
   }



#' @title Adaptive lasso in GEEs (single fit)
#' 
#' @description Fits single penalized GEEs to achieve sparsity using an adaptive lasso penalty, assuming either an independence, an unstructured, or a reduced-rank working correlation matrix. 
#' This function is designed to used after the regularization path has been constructed using the \code{adlgee_regpath} function. That is, after applying \code{adlgee_regpath}, this function is then used to calculate the GEE fit at specific values of the tuning parameter, lambda.
#' 
#' @param adlgee_regpath An object of class "adlgee_path".
...
#' @param num_cores The number of cores to use as part of computation. 
#' 
#' @return A object of class "adlgee_fit" with the following values, as appropriate:
#' \item{call:}{The function call.}
...
adlgee_fit <- function(adlgee_regpath, lambda, family = gaussian(), corstr = "independence", rank = 2) {
   if(length(lambda) > 1)
      stop("Only a scalar value of the tuning parameter, lambda, is permitted.")
   
   out <- .adlgee_fit_manual(yvec = adlgee_regpath$singlefit_quantities$yvec, 
                            bigX = adlgee_regpath$singlefit_quantities$bigX,
                            lambda = lambda,
                            penalty_weights = adlgee_regpath$singlefit_quantities$penalty_weights,
                            num_resp = adlgee_regpath$num_resp,
                            num_units = adlgee_regpath$num_units,
                            family = family, 
                            corstr = corstr,
                            rank = rank,
                            response_names = adlgee_regpath$singlefit_quantitles$response_names,
                            covariate_names = adlgee_regpath$singlefit_quantitles$covariate_names,
                            control = adlgee_regpath$singlefit_quantities$control)
...
.adlgee_fit_manual <- function(yvec, bigX, lambda = 0, penalty_weights, 
                          num_resp, num_units, family = gaussian(), corstr = "independence", rank = 2, start_coef = NULL, 
                          response_names = NULL, covariate_names = NULL, getalambdaseq = FALSE, control) {
   
   ##-----------------------
   ## Opening checks and set up
   ##-----------------------
   num_X <- ncol(bigX)/num_resp
   ...
   new_reparametrized_coef <- cw_reparametrized_coef <- as.vector(t(new_coef_mat))
   new_disp_param <- cw_disp_param <- rep(0.5, num_resp)
   new_power_param <- cw_power_param <- rep(1.2, num_resp)
   
   start_fit <- list(linear_predictor = as.vector(bigX %*% cw_reparametrized_coef))
   ...
   while(counter < control$maxit) {
      ...
      if(counter > 1) {
         cw_linear_predictor <- as.vector(bigX %*% cw_reparametrized_coef)
      ...
      if(penalty_type == "adaptive_lasso") {
         get_update_coef <- .update_coef_glmnet(cw_reparametrized_coef = cw_reparametrized_coef, yvec = yvec, bigXtrans = bigX, 
      ...
      new_coef_mat <- matrix(new_reparametrized_coef, nrow = num_resp, byrow = TRUE)

      new_linear_predictor <- as.vector(bigX %*% new_reparametrized_coef)
   ...
   if(getalambdaseq) {
      formregpath <- .update_coef_glmnet(cw_reparametrized_coef = cw_reparametrized_coef, yvec = yvec, bigXtrans = bigX, 
